...
hunt(Life,Time,OffSize,DefSize,Radius,MapWidth,EnemyInSight):-
	...
	being_attacked(Time),
	not(EnemyInSight),
	jpl_call('sma.actionsBehaviours.PrologHeightRandomBehavior',executeHunt,[],@(void)).
...
attack(EnemyInSight):-
	EnemyInSight,
	jpl_call('sma.actionsBehaviours.PrologHeightRandomBehavior',executeAttack,[],@(void)).
